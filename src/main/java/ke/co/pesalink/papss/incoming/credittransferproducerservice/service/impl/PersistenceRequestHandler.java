...
public class PersistenceRequestHandler implements RequestQueueHandler {
    private final AppConfig appConfig;
    private final RabbitTemplate rabbitTemplate;

    private final Logger logger = LoggerFactory.getLogger(PersistenceRequestHandler.class);
    @Override
    public void queueRequest(TransactionRequest transactionRequest) {
        logger.info("Received request {} to send to {} queue", transactionRequest, appConfig.getPeristenceQueue());
        try {
            this.rabbitTemplate.convertAndSend(appConfig.getAdapterExchange(), appConfig.getPersistenceRoutingKey(), transactionRequest);

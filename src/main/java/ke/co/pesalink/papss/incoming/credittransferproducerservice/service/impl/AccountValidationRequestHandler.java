...
public class AccountValidationRequestHandler implements RequestQueueHandler {

    private final AppConfig appConfig;
    private final RabbitTemplate rabbitTemplate;

    private final Logger logger = LoggerFactory.getLogger(AccountValidationRequestHandler.class);
    @Override
    public void queueRequest(TransactionRequest transactionRequest) {
        logger.info("Received status query request {} to send to {} queue", transactionRequest, appConfig.getAccountValidationQueue());
        try {
            this.rabbitTemplate.convertAndSend(appConfig.getAdapterExchange(), appConfig.getAccountValidationRoutingKey(), transactionRequest);
